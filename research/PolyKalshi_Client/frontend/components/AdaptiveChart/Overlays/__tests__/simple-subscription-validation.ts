...
class SubscriptionValidator {
   ...
   */
  parseSubscriptionId(subscriptionId: string): ParsedSubscription | null {
   ...
   */
  validateBaselineSubscriptions(): void {
    console.log('\n🧪 Validating all baseline subscription IDs...\n')
    
    const seriesTypes: SeriesType[] = ['YES', 'NO']
    const timeRanges: TimeRange[] = ['1H', '1W', '1M', '1Y']
    ...
    for (const seriesType of seriesTypes) {
      for (const timeRange of timeRanges) {
        const subscriptionId = BASELINE_SUBSCRIPTION_IDS[seriesType][timeRange]
        totalTests++
        ...
        if (parsed) {
          ...
          const expectedSide = seriesType.toLowerCase() as 'yes' | 'no'
   ...
   */
  testSubscriptionIdCompatibility(): void {
   ...
   */
  testRangeSwitching(): void {
    console.log('\n🧪 Testing range switching logic...\n')
    
    const originalId = BASELINE_SUBSCRIPTION_IDS.YES['1H']
    ...
    const newRanges: TimeRange[] = ['1W', '1M', '1Y']
    
    for (const newRange of newRanges) {
      // This is how the series would generate a new subscription ID
      const newId = BASELINE_SUBSCRIPTION_IDS[parsed.side.toUpperCase() as SeriesType][newRange]
